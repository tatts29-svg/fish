...
echo ===============================================================
echo.
echo [0/14] COMPLIANCE FLAGS (new gear only - leaves your edits alone)
%PYCMD% SETUP_COMPLIANCE_FLAGS.py
echo.
echo [1/14] ALIGN EXCEL DATA
...
echo.
echo [8/14] CLEAN REGISTER
%PYCMD% build_clean_report.py
echo.
echo [9/14] OUTLOOK DRAFTS (rebuild all of today's)
...
echo.
echo [10/14] DAILY EMAIL PACKS (company folders, drafts, record)
%PYCMD% BUILD_DAILY_EMAIL_PACKS.py
echo.
echo [11/14] YOUR OWN REPORTS - every idea you saved
%PYCMD% report_designer.py --all
echo.
echo [12/14] THE EXTRA PAGES (clock, hidden list, plant, utilisation)
...
echo.
echo [14/14] REPORTS ON YOUR PHONE - the money ones locked under your code
