#!/usr/bin/env python3
"""v6.79 (approved 26 Sep 2026: on the live v6.73 with patch_v677d only; the v6.77 dashboard fixes were NOT approved) - THE RUNNING SHEET and THE QUESTIONS PAGE (Andrew, 26 Sep 2026).
...
/* the fencing types behind the programme by today, each in its own unit (standalone: v6.77's helpers are not on this page) */
function rsFenceGaps(ft){ return (ft || []).filter(t => t.planned != null && t.done < t.planned).map(t => ({name: t.name, unit: t.unit, done: t.done, planned: t.planned, short: Math.round((t.planned - t.done) * 100) / 100})); }
function rsGapName(g){ const n = String(g.name || '').replace(/^Fence — /, ''); return n.charAt(0).toUpperCase() + n.slice(1); }
function questionsList(){
 ...
 try { rsFenceGaps(fenceTypes(progressAsOf(td))).forEach(g => add('Fencing', 'fe-gap-' + ourSlug(g.name), `${rsGapName(g)}: ${fmtNum(g.done)} of ${fmtNum(g.planned)}${g.unit === 'm' ? ' m' : ''} due by today — what is the plan to catch up, and by when?`, 'Early work on other fencing types does not make it up.', 'fencing')); } catch (e) {}
