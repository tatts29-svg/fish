...
export function buildCrew({service = null, wheels = [], kit = null} = {}) {
  ...
  for (const id of ['operator', 'mechanic', 'tech', 'engine', 'lead', 'safety']) { root.add(men[id].fig.root); men[id].floor = floorAt; }
  /* the props */
  ...
  function placeAll() {
    ...
    seatOperator(); seatDriver(); men.safety.place(-4.8, 2.6, yawTo(4.8, -2.6));
  }
  resetProps(); placeAll();
