...
import {buildExhaustFX} from './exhaust-fx.js';import {buildOcclusion,buildWheelBlur} from './view-fx.js';let exfx=null,wfx=null,occl=null,dynoSpin=0,dynoT=0,autoShifting=false,manualShiftAt=-99,autoGearAt=0,liftFor=0,frameCost=16,fastFrames=false,fastLocked=false;   /* v5.84 — fire and heat */
const $=id=>document.getElementById(id),reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;
const materials=makeMaterials(),body=buildCarBody(T,materials),engine=buildPowertrain(T,materials);fitCarMechanics(T,body,engine);try{exfx=buildExhaustFX(T,engine,{light:!/Mobi|Android/i.test(navigator.userAgent)});}catch(e){console.warn('Exhaust effects unavailable:',e.message);}try{wfx=buildWheelBlur(T,body.wheels);}catch(e){console.warn('Wheel blur unavailable:',e.message);}const carAssets=[...body.removable,...engine.removable];/* the cockpit's live parts — the display, the switches, the wheel's buttons, the lever (car-cockpit.js, built by the fit) */const cockpit=engine.removable.find(p=>p.id==='cockpit').cockpit;
...
function shiftTo(g){if(!autoShifting)manualShiftAt=dynoT;g=Math.max(0,Math.min(COCKPIT.gears.length-1,g));const dir=Math.sign(g-cabin.gear);if(g===cabin.gear){sfx('ui-denied',.5);toast(g?'Top gear already.':'Neutral already.');return;}cabin.gear=g;cockpit.setGear(g,dir);if(engine.setGear)engine.setGear(g);drive.shift();shiftHand=1;sfx('dog-engage',.7);if(!autoShifting)toast(g?`Gear ${COCKPIT.gears[g]}${dir>0?' — pull back for the next':' — push forward for the one below'}.`:'Neutral.');updateUI();}
