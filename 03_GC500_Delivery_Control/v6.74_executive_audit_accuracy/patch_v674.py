#!/usr/bin/env python3
"""v6.74 - THE EXECUTIVE AUDIT, PART 1: WHAT THE NUMBERS MEAN (the CEO-presentation audit of 26 Sep 2026, DATA-01..10).
...
def patch(path, need):
 ...
 <div class="cwpc"><span${C.refsNoQty ? ' title="provisional — each reference awaiting a quantity counts as one until it is confirmed"' : ''}><i class="on"></i>${pc1(C.pct)}% recorded${C.refsNoQty ? ' · provisional' : ''}</span>""", 'whole tile')
    ...
    t = R("""C.toRace > 1 ? 'days to race' : C.toRace === 1 ? 'day to race' : C.toRace === 0 ? 'race day' : 'days since race'}</span></div>` : '';""",
          """C.toRace > 1 ? 'days to the event' : C.toRace === 1 ? 'day to the event' : C.toRace === 0 ? 'event opens' : 'days since it opened'}</span></div>` : '';""", 'crace')
    t = R("""if (race && iso < race) { lab = 'Race day · ' + fmtDay(race).dow + ' ' + fmtDay(race).dm; n = days(iso, race); unit = n === 1 ? 'DAY' : 'DAYS'; sub = wk ? weekWords(wk).split(' · ').slice(0, 2).join(' · ') : 'before the programme'; }""",
          """if (race && iso < race) { lab = 'Event opens · ' + fmtDay(race).dow + ' ' + fmtDay(race).dm; n = days(iso, race); unit = n === 1 ? 'DAY' : 'DAYS'; sub = wk ? weekWords(wk).split(' · ').slice(0, 2).join(' · ') : ((programmeDay(iso) || {}).short || 'before the programme'); }""", 'pod label')
    t = R("""(race ? ` · race day ${fmtDate(race)}` : '')""", """(race ? ` · the event opens ${fmtDate(race)}` : '')""", 'pod title')
