...
function pump() {
  ...
  while (running < 8 && queue.length) {
    ...
    fetch(url, {signal: ac.signal, cache: 'force-cache'}).then(r => { if (!r.ok) throw new Error('tile ' + r.status); perf.tileBytes += +(r.headers.get('content-length') || 0); return r.blob(); })
      .then(b => createImageBitmap(b)).then(bm => { tiles.set(t.key, {bm, at: performance.now()}); perf.tilesFetched++; evict(); requestPaint(); })
      .catch(e => { if (e.name !== 'AbortError') { tiles.set(t.key, {bm: null, at: performance.now(), err: String(e)}); if (/401|403/.test(String(e))) keyProblem(); } })
...
function drawTiles(v) {
  ...
  for (const R of regionsForTiles()) {
    ...
    for (let ty = ty0; ty <= ty1; ty++) for (let tx = tx0; tx <= tx1; tx++) {
      ...
      if (t && t.bm) { t.at = performance.now(); ctx.drawImage(t.bm, tx * TILE, ty * TILE, TILE + .5, TILE + .5); }
      else { missing++; if (!t) requestTile(z, tx, ty, Math.hypot(tx - cxT, ty - cyT)); const p = parentTile(z, tx, ty); if (p) ctx.drawImage(p.bm, p.sx, p.sy, p.sw, p.sw, tx * TILE, ty * TILE, TILE + .5, TILE + .5); }
    }
...
function draw() {
  ...
  if (mode === 'original') { ctx.fillStyle = '#fff'; ctx.fillRect(0, 0, SHEET_W, SHEET_H);
    ...
    if (legendOn && preview) { ctx.save(); ctx.beginPath(); ctx.rect(0, 1466, SHEET_W, SHEET_H - 1466); ctx.clip(); ctx.fillStyle = '#fff'; ctx.fillRect(0, 1466, SHEET_W, SHEET_H - 1466); ctx.drawImage(preview, 0, 0, SHEET_W, SHEET_H); ctx.restore(); }
    drawTiles(v); ctx.setTransform(...S2D);
  }
  let vt = null;
  if (mode !== 'satellite' && !(mode === 'original' && isPreviewEnough(v) && !(UNDER.length && PYR))) {
  ...
  if (ready) { if (mode === 'satellite') quality('sharp', 'Satellite only' + (tileZoomAny(v) ? ' · imagery enlarged beyond its detail' : ''));
    else if (mode === 'original' && isPreviewEnough(v) && !(UNDER.length && PYR)) quality('', 'Full-sheet overview');
...
function alignmentPanel() {
  ...
  const c = GEO.main.check, i = GEO.inset.check; $('alDot').className = 'dot ' + (c && c.rms_m < 1.5 ? 'ok' : '');
  $('alText').textContent = `Image registration · main plan check ${c.rms_m.toFixed(2)} m rms over ${c.n} held-out points (worst ${c.max_m.toFixed(2)} m)`;
...
async function boot() {
  try {
    ...
    preview = await new Promise((res, rej) => { const im = new Image(); im.onload = () => res(im); im.onerror = rej; im.src = 'assets/original-preview.webp'; }); $('miniImg').src = preview.src; requestPaint();
    ...
    const [geo, cls] = await Promise.all([fetch('assets/georeferencing.json').then(r => r.ok ? r.json() : null), fetch('assets/classification.json').then(r => r.ok ? r.json() : null)]);
    ...
    const readyMsg = await new Promise((res, rej) => { const h = e => { if (e.data.t === 'ready') { worker.removeEventListener('message', h); res(e.data); } else if (e.data.t === 'error') { worker.removeEventListener('message', h); rej(new Error(e.data.message)); } }; worker.addEventListener('message', h); worker.postMessage({t: 'load', url: new URL('assets/drawing-scene.bin', location.href).href, underlay: [...UNDERLAY]}); });
    P = {meta: readyMsg.meta, labels: readyMsg.labels, count: readyMsg.count}; perf.sceneMs = readyMsg.ms;
    const [pyr, under] = await Promise.all([fetch('assets/vt/manifest.json').then(r => r.ok ? r.json() : null).catch(() => null), fetch('assets/underlay/manifest.json').then(r => r.ok ? r.json() : null).catch(() => null)]);
    if (pyr && pyr.levels && pyr.levels.length) { PYR = pyr; PYR_MAX = Math.max(...pyr.levels.map(l => l.L)); } if (under) UNDER = under.items;
