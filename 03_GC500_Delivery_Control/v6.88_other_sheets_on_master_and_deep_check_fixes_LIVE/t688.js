...
(async () => { const b = await chromium.launch({executablePath: '/opt/pw-browsers/chromium', args: ['--no-sandbox']});
  ...
  for (const k of ['wb', 'gate', 'ep', 'screen', 'gens', 'iface', 'wcx', 'vms']) {
    ...
    if (k === 'wb' || k === 'screen') await p.screenshot({path: `locfind/v688_${k}_${vp.width}.png`});
  }
  ...
  await p.screenshot({path: `locfind/v688_pop_${vp.width}.png`});
  ...
  await p.evaluate(() => { state.q = ''; renderMap(); openAsset('P27'); }); await p.waitForTimeout(4000);
